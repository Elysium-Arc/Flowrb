...
RSpec.describe Piperb::DAG do
  let(:dag) { described_class.new }

  def make_step(name, deps = [])
    Piperb::Step.new(name, depends_on: deps) { 'result' }
  end
  ...
  describe 'cycle detection edge cases' do
    ...
    it 'detects cycle with 4 nodes' do
      ...
      expect { dag.validate! }.to raise_error(Piperb::CycleError)
    end
    ...
    it 'handles graph with cycle and missing dependency' do
      ...
      expect { dag.validate! }.to raise_error(Piperb::MissingDependencyError)
    end
  end
  ...
  describe 'missing dependency edge cases' do
    it 'detects multiple missing dependencies in one step' do
      ...
      expect { dag.validate! }.to raise_error(Piperb::MissingDependencyError) do |error|
