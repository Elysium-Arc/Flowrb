...
RSpec.describe Piperb::Cache do
  describe Piperb::Cache::MemoryStore do
  ...
  describe Piperb::Cache::FileStore do
    subject(:store) { described_class.new(cache_dir) }

    let(:cache_dir) { File.join(Dir.tmpdir, "piperb_cache_test_#{Process.pid}") }
...
RSpec.describe 'Pipeline Caching' do # rubocop:disable RSpec/DescribeClass
  let(:cache_dir) { File.join(Dir.tmpdir, "piperb_pipeline_cache_#{Process.pid}") }
  ...
  describe 'cache with file store' do
    it 'persists cache across pipeline instances' do
      ...
      new_file_store = Piperb::Cache::FileStore.new(cache_dir)
