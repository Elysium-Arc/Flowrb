...
RSpec.describe Piperb::StepResult do
...
RSpec.describe Piperb::Result do
  ...
  describe 'multiple step results' do
    let(:step_a) { Piperb::StepResult.new(step_name: :a, output: 1, status: :success) }
    let(:step_b) { Piperb::StepResult.new(step_name: :b, output: 2, status: :success) }
    let(:step_c) { Piperb::StepResult.new(step_name: :c, output: 3, status: :success) }
    ...
    it 'fails if any step failed' do
      failed = Piperb::StepResult.new(step_name: :failed, error: StandardError.new)
  ...
  describe '#to_h' do
    ...
    it 'includes nested step results' do
      step = Piperb::StepResult.new(step_name: :test, output: 'value', status: :success)
  ...
  describe '#[]' do
    ...
    it 'works with symbol key' do
      step = Piperb::StepResult.new(step_name: :test, output: 42, status: :success)
      result = described_class.new(step_results: { test: step })
  ...
  describe '#inspect' do
    it 'shows step count' do
      steps = {
        a: Piperb::StepResult.new(step_name: :a, status: :success),
        b: Piperb::StepResult.new(step_name: :b, status: :success),
        c: Piperb::StepResult.new(step_name: :c, status: :success)
      }
  ...
  describe 'success determination' do
    it 'is failed if result has error even with successful steps' do
      step = Piperb::StepResult.new(step_name: :ok, output: 1, status: :success)
      result = described_class.new(
    ...
    it 'is failed if any step failed even without result error' do
      ok_step = Piperb::StepResult.new(step_name: :ok, output: 1, status: :success)
      failed_step = Piperb::StepResult.new(step_name: :failed, error: StandardError.new)
    ...
    it 'is successful only when no error and all steps successful' do
      step_a = Piperb::StepResult.new(step_name: :a, output: 1, status: :success)
      step_b = Piperb::StepResult.new(step_name: :b, output: 2, status: :success)
