...
module Piperb
  ...
  class StepResult
    ...
    def inspect
      "#<Piperb::StepResult step=#{step_name.inspect} status=#{status.inspect} duration=#{duration&.round(4)}>"
    end
  end
  ...
  class Result
    ...
    def inspect
      ...
      "#<Piperb::Result status=#{status} steps=#{step_results.size} duration=#{duration&.round(4)}>"
    end
  end
end
